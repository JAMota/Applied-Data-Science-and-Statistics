...
netherlandsSP = getData("GADM", country = "NLD", level = 0)
...
# grid_points_sp <- SpatialPoints(grid_points, proj4string = CRS(proj4string(netherlandsSP)))
# 
# # Intersect grid points with the Netherlands' boundaries
# grid_points_netherlands <- grid_points_sp[netherlandsSP, ]
...
# Kriging with the fitted variogram model
krigeControl <- krige.control(obj.m = Matren2.5linearREMLmaximumLikelihoodNetherlandsInitial10.1)
krigeResults <- krige.conv(precipitationNetherland_geoR, locations = grid_points_netherlands, krige = krigeControl)
