...
priorMean = Matren1.0linearREMLmaximumLikelihoodNetherlandsInitial10.1$beta
priorSigmaSQ = Matren1.0linearREMLmaximumLikelihoodNetherlandsInitial10.1$sigmasq
priorNugget = Matren1.0linearREMLmaximumLikelihoodNetherlandsInitial10.1$nugget
priorCovariance = Matren1.0linearREMLmaximumLikelihoodNetherlandsInitial10.1$cov.pars


 ?prior.control
# <- prior.control(mean.prior = priorMean, 
#                             var.prior = priorSigmaSQ, 
#                             nugget.prior = 0, 
#                             cov.prior = priorCovariance)
# 
# 
# priors <- prior.control(mean.prior = c(priorMean), var.prior = c(priorSigmaSQ), 
#                         nugget.prior = c(0), cov.prior = c(priorCovariance))


kriging_result <- krige.bayes(geodata = precipitationNetherland_geoR, locations = spatialPointsABC, priors = priorParametersNoNugget)
...
priorParametersNoNugget = prior.control(
  beta.prior = "normal", # Normal prior for mean parameter β
  beta = priorMean, # Mean hyperparameter for β
  beta.var = priorSigmaSQ, # Covariance hyperparameter for β

  sigmasq.prior = "sc.inv.chisq", # Scaled inverse Chi-squared prior for σ²
  sigmasq = 2, # Fixed value for σ², only used if sigmasq.prior = "fixed"
  df.sigmasq = 3, # Degrees of freedom for σ² prior

  phi.prior = "uniform", # Uniform prior for range parameter ϕ
  phi = priorCovariance, # Range for uniform prior or hyperparameter ν for exponential prior

  tausq.prior = "fixed", # Fixed value for nugget effect τ²
  tausq = 0 # Fixed value for τ²
)


ex.grid <- as.matrix(expand.grid(seq(50.5,53.5,l=6), seq(3.5,7,l=7)))

ex.bayes <- krige.bayes(geodata = precipitationNetherland_geoR, loc = ex.grid,
                        model = model.control(cov.m = "matern", kappa = 1.5),
                        prior = priorParametersNoNugget)
