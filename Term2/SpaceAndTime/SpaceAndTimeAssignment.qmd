...
Here we create a ts object with frequency 366 because 2012 does indeed have the 29th of February.
...
The model does seem stationary however it is not clear enough since we only have 1 year worth of data
...
buildFun = function(x) {
dlmModPoly(order = 2, dV = exp(x[1]), dW = c(0, exp(x[2]))) +
dlmModSeas(frequency = 366, dV = 0, dW = c(exp(x[3]), rep(0,364)))
}

linearDLMNapa = dlmMLE(tsCaliforniaNapaTemp, parm = c(0,0,0), build = buildFun)

linearDLMNapa$par

fittedLinearDLMNapa = buildFun(linearDLMNapa$par)

V(fittedLinearDLMNapa)

W(fittedLinearDLMNapa)
