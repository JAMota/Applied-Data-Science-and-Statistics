...
precip_variog = variog(precip_geoR, option='bin', max.dist = 2)
...
krigingVariogramFitted <- variofit(precip_variog)
...
# Create the ggplot
plotARIMA <- ggplot(combined_data, aes(x = Date, y = Temperature, color = Type, group = 1)) +
  geom_line() +
...
plotARIMA <- plotARIMA + scale_x_discrete(
  breaks = combined_data$Date[c(TRUE, rep(FALSE, 3))],
  labels = combined_data$Date[c(TRUE, rep(FALSE, 3))]
)
