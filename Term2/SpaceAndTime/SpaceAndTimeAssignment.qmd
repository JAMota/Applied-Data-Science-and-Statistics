...
First we will start by using the predictions using the variogram
...
krige_control = krige.control(type.krige = "OK", cov.model = krigingVariogramFittedMatrén1.0$cov.model, cov.pars = krigingVariogramFittedMatrén1.0$cov.pars)


# Kriging with the fitted variogram model
krige_result = krige.conv(precipitationNetherland_geoR, locations = spatialPointsABC, krige = krige_control)

# Extract predictions from the kriging results
predictions = krige_result$predict

# Compare the predicted values with the actual precipitation values
actual_precip = c(89.1, 95.9, 147.2)
comparisonvariogram = data.frame(actual_precip, predictions)
