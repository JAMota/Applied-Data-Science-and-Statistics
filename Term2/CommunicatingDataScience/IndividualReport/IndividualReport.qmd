...
propose strategies for adaptation and mitigation. exhaustion of code
fisheries (fishing quotas, alternative fishing, like fishlike code
pangaceas)

### Scope and limitations

The report is limited in scope to the impacts of climate change on
Atlantic cod fisheries in Iceland, Norway and Danish Faroe Islands from
1950 to 2010. The report does not cover other fish species or historical
...
which may be limited in scope or availability. While the report provides
...
policy recommendations. The report aims to provide a comprehensive
...
V. Conclusion A. Summary of Findings B. Implications for Cod Fisheries
and Fishing Communities C. Future Directions for Research and Action

This improves the world because goverments can take this data to plan
for longevitie of cod supplies for future generations
...
IcelandSeaSurfaceTemperatureYearly = IcelandSeaSurfaceTemperatureYearly %>% select(-(`DATE-YMD`))
...
IcelandSeaSurfaceTemperatureRecently = IcelandSeaSurfaceTemperatureRecently %>% select(-(...3))
...
train_size <- floor(nrow(IcelandTimeSeriesDf) * 0.8)
train <- IcelandTimeSeriesDf[1:train_size, ]
test <- IcelandTimeSeriesDf[(train_size + 1):nrow(IcelandTimeSeriesDf), ]
...
forecast_values <- predict(model, n.ahead = length(test))

# Extract the forecasted values for the second variable (number of fishes)
forecast_values_fish <- forecast_values$fcst[,2]

# Calculate the error metrics
mape <- mean(abs(test[,2] - forecast_values_fish)/test[,2])
rmse <- sqrt(mean((test[,2] - forecast_values_fish)^2))
