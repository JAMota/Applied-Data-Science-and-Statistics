...
are always positive. which makes sense for poisson and not for normal //TODO redo this pls
...
<!-- //TODO rewrite because monkey sees line -->
...
Now that we have extended the improved Poisson into a negative Binomial, we will compare this new model against the previously improved Poisson model which we already determined altough not a good model it is to be prefered over the improved Gaussian model.
...
1 - pchisq(model2Improved$deviance, model2Improved$df.residual)
```
The p-value is larger than 0.05, meaning that the negative binomial is a good fit to the data. 

Lastly from the Akaike Information Criterion test we can see that the Negative Binomial has the best goodness of fit per complexity and since it has the same complexity has the improved Poisson, it simply performs better
```{r tidy=TRUE, tidy.opts=list(width.cutoff=72)}
