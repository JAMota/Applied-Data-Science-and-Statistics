...
xx <- seq(83, 94, len=200)

##predict Poisson
predPoisson = predict(model2, newdata = aids, type="link",se.fit = T)

## Mean and Confidence Intervals estimation
muPoisson = 1/predPoisson$fit
muPoisson_upper = 1/(predPoisson$fit+1.96*predPoisson$se.fit)
muPoisson_lower = 1/(predPoisson$fit-1.96*predPoisson$se.fit)
...
## Mean and Confidence Intervals estimation
muNormal = 1/predNormal$fit
muNormal_upper = 1/(predNormal$fit+1.96*predNormal$se.fit)
muNormal_upper = 1/(predNormal$fit-1.96*predNormal$se.fit)
...
# Predicting when supplement is OJ
predsOJ <- predict(model2,
newdata = aids,
type = "link",
se.fit=T)

# Estimating mean and CI
muOJ <- 1/predsOJ$fit
muOJ_upper <- 1/(predsOJ$fit+1.96*predsOJ$se.fit)
muOJ_lower <- 1/(predsOJ$fit-1.96*predsOJ$se.fit)

# Predicting when supplement is VC
predsVC <- predict(model1,
newdata = aids,
type = "link",
se.fit = T)
...
# Plotting data
plot(aids$date,aids$cases,pch=20,
xlab = 'Date', ylab = 'Number of aids cases')

#Adding predictions and CIs
lines(xx,muOJ)
lines(xx,muOJ_upper,lty=2)
lines(xx,muOJ_lower,lty=2)
lines(xx,muVC,col="red")
lines(xx,muVC_upper,lty=2,col="red")
lines(xx,muVC_lower,lty=2,col="red")
