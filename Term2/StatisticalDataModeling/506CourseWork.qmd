...
As we can see from the Residual bs fitted model, there seems to be pattern in how the model overfits and underfits the data in a way the would require more flexibility from a multi polynomial equation, like a quadratic or cubic term.
...
```
The Poisson model residuals seem to follow a quadratic function, this mean it required a quadratic or higher term
 polynomial for the flexibility to better fit the data.
 
 
## Question 2 e)
 
 As we commented on the previous  
```{r tidy=TRUE, tidy.opts=list(width.cutoff=72)}

#Fitting model 2 improvement
model2Improved = glm(cases ~ date + date^2, data = aids, family = poisson(link='log'))

#Fitting model 1 improvement
model1Improved = glm(cases ~ date, data = aids, family = gaussian(link='log'))
