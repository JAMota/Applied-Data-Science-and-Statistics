...
muNormal_upper = predNormal$fit+qnorm(1-1.96/2)*predNormal$se.fit ## p$fit+qnorm(1-a/2)*p$se.fit
muNormal_lower = predNormal$fit-qnorm(1-1.96/2)*predNormal$se.fit ## p$fit-qnorm(1-a/2)*p$se.fit
...
As we can see from the Residual bs fitted model, there seems to be pattern in how the model overfits and underfits the data in a way the would require more flexibility from a multi polynomial equation, like a quadratic or cubic.
