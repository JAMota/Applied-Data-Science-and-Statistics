...
# Log-likelihood
result = (-(n/2)*log(2*pi)) -((n/2)*log(sigma^2)) -(1/(2*(sigma^2))) * (sum(nlmodel$y-(theta1*nlmodel$x)/(theta2+nlmodel$x)))
...
when x -> 1 y is approximately 200
...
# Estimating the MLE
out <- nlm(myLike,
  p = c(50,0.01,10), #c(50,0.01,10), c(10,0.24,30)
  hessian = T,
  iterlim = 10000,
  steptol = 1e-10)
