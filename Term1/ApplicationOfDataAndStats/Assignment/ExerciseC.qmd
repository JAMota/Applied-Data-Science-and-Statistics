...
As the cluster 4 is the most numerous by far it can be considered the baseline for daily power demand of the "common" substation seeing as it represents approximately 65% of all substations. This cluster is also obviously the one that runs with the lowest average demand during the day, running significantly lower in power demand than the other clusters. A possible explanation for this lower level of power demand and high level of substations is that this cluster could be representing a majority of business serving substations, that usually have less power in comparison the industries. A high level of business costumers would also help explaining the significantly less power usage during the night as business are less likely to have nocturnal working hours than industries. Therefore I belive we should give cluster 4 the name of baseline_business.

Next we analyse cluster 2 visualization has the pair of cluster 4. From the graph we can see that cluster 2 is the top or close to the top on the average demand during the night time, having a earlier morning increase than the the other clusters
...
As we can observe by the summary statistics 



Here we actually change the name of all the clusteres to the suggested ones.

```{r tidy=TRUE, tidy.opts=list(width.cutoff=72)}
colnames(January_2013GroupedClusterAverageVertical)[4] = "baseline_business"
