...
for (i in 1:nrow(tenMinuteMaximum)) {
  ...
  maxVal = tenMinuteMaximum$`00:00`[i]
  ...
  maxVal <- apply(df, 1, function(x) {if(!(rownames(df) %in% c("2","3"))) max(x)})

  
  
}

## afterwards we devide every 10 minute interval by that maximum value

isBigger = apply(tenMinuteMaximum[1,], 1, max)
