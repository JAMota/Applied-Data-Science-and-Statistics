...
Lastly, age after 40 years of age seems to a very big variance on its relationship with weight.
...
```
As we can see from the multiple R-squared value and adjusted R-squared a simple linear model already explains approximately 88,8% of the variance that influence a person's weight.
...
##logging the exponential height and weight variables 

dataWeightHeight$heightLogged = log(dataWeightHeight$height)
dataWeightHeight$weighttLogged = log(dataWeightHeight$weight)


modelLoggingHeightWeight = lm(weighttLogged ~ heightLogged  + age + isMale , data = dataWeightHeight)

summary(modelLoggingHeightWeight)
